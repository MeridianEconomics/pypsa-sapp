...
rule build_renewable_profiles:
    params:
        ...
        shape_layer = config['build_shape_options']['gadm_layer_id'],
        countries=config["countries"],
        alternative_clustering=config["cluster_options"]["alternative_clustering"],
    input:
...
rule cluster_network:
    ...
    input:
        ...
        custom_busmap=(
            PYPSAEARTH_DIR + "data/custom_busmap_elec_s{simpl}_{clusters}.csv"
    ...
    output:
        ...
        busmap=PYPSAEARTH_DIR + "resources/" + RDIR + "bus_regions/busmap_elec_s{simpl}_{clusters}.csv",
        linemap=PYPSAEARTH_DIR + "resources/" + RDIR + "bus_regions/linemap_elec_s{simpl}_{clusters}.csv",
    log:
    ...
    input:
        ...
        **branch(sector_enable["heat"], HEAT),
        ...
        airports=branch(
            ...
            PYPSAEARTH_DIR + "resources/" + SECDIR + "airports.csv",
        ),
        ports=branch(sector_enable["shipping"], PYPSAEARTH_DIR + "resources/" + SECDIR + "ports.csv"),
...
if config["foresight"] == "myopic" and config['enable_sector_coupling']:
...
if config["foresight"] == "myopic" and not config['enable_sector_coupling']:
    ...
    rule solve_elec_network_myopic:
        params:
            solving=config["solving"],
            foresight=config["foresight"],
            planning_horizons=config["scenario"]["planning_horizons"],
    ...
    rule solve_elec_networks_myopic:
        input:
            networks=expand(
                RESDIR
                + "postnetworks/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_{planning_horizons}_{discountrate}_{demand}_elec.nc",
