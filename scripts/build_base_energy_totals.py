    ...
    if snakemake.params.update_data:
        ...
        try:
            for key, value in d.items():
                ...
                with urlopen(zipurl) as zipresp:
                    with ZipFile(BytesIO(zipresp.read())) as zfile:
                        zfile.extractall(
                            os.path.join(PYPSAEARTH_DIR, "data/demand/unsd/data")
                        )
            ...
            download_gdrive(
                ...
                dest_path=os.path.join(PYPSAEARTH_DIR, "data/demand/unsd/unsd.zip"),
                unzip=True,
                overwrite=True,
            )

            # clean up __MACOSX folder if it exists
            macosx_root_path = os.path.join(PYPSAEARTH_DIR, "data/demand/unsd/__MACOSX")
